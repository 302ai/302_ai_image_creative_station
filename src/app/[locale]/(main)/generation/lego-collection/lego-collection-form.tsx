...
import { legoCollectionPrompt } from "./prompt";
...
export default function LegoCollectionForm() {
  ...
  const handleGenerate = async () => {
    if (!imageForm) {
      toast.error(t("lego-collection.warning.image"));
      return;
    }

    await generateWithImage({
      rawPrompt: "",
      prompt: legoCollectionPrompt(),
      imageData: imageForm,
      shouldUseImageInput: true,
      type: "lego_collection",
    });
  };
        ...
        <div className="mt-4 flex w-full flex-1 flex-col justify-end space-y-12 md:ml-4 md:mt-0 md:w-1/2">
          <div className="mb-4 flex w-full flex-col space-y-2">
            <Label className="mb-2 mr-1 text-sm font-medium sm:mb-0">
              {t("lego-collection.label.description")}
            </Label>
          </div>
