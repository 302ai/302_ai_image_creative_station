...
export default function OriginalProductImageForm() {
  ...
  const handleGenerate = async () => {
    ...
    const defaultValues = {
      text: t("original-product-image.defaultValue.text"),
    };
    await generateImg({
      rawPrompt: text || defaultValues.text,
      prompt: originalProductImagePrompt(text || defaultValues.text),
      isOptimize: false,
      customOptimizePrompt: originalProductImagePrompt(
        text || defaultValues.text
      ),
      type: "original_product_image",
    });
  };
